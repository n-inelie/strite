...
static inline void render_status_bar(struct stage *stage, struct cursor *cursor) {
...
static inline void render_text(struct stage *stage, struct cursor *cursor) {
    ...
    while(index < stage->page->text_len) {
        cursor_move(cursor, stage->min_y + y_offset, stage->min_x + stage->line_nr_width + x_offset);
        x_offset++;
        if (text[index] == '\n') {
            y_offset++;
            x_offset = 0;
